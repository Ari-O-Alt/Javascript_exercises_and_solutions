...
const printInput = () => {
  ...
  if (userInput === answer) {
    ...
    const text = document.createTextNode(
      `Not matched! The correct number is bigger than ${userInput} and is between ${min} and ${max}!`
    );
    textTarget.innerHTML = " ";
    textTarget.appendChild(text);
  } else {
    max = userInput - 1;
    const text = document.createTextNode(
      `Not matched! The correct number is smaller than ${userInput} and is between ${min} and ${max}!`
    );
    textTarget.innerHTML = " ";
    textTarget.appendChild(text);
  }
